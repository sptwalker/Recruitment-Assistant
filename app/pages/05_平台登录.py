...
with tabs[0]:
    ...
    platform_options = {
        "BOSS直聘": {"adapter": BossAdapter, "snapshot_url": "https://www.zhipin.com/web/chat/index"},
    ...
    if platform_name != "BOSS直聘":
        ...
        st.info("BOSS 已切换为外部真实浏览器登录方案：请先在 Chrome/Edge 正常登录 BOSS，再导出 zhipin.com Cookie 导入本系统。")
    if platform_name == "BOSS直聘":
        st.caption("BOSS 专用稳定模式：不再使用 Playwright 打开登录页。请在外部真实浏览器完成登录后导出 Cookie，再导入本系统。")
        st.markdown(
            """
            操作步骤：
            1. 用日常 Chrome/Edge 打开 `https://www.zhipin.com/` 并完成登录/安全验证。
            2. 使用浏览器 Cookie 导出工具导出 `zhipin.com` 域名 Cookie 的 JSON 数组。
            3. 粘贴到下方输入框并点击“导入 BOSS Cookie 登录态”。
            4. 再点击“检测是否已登录”验证。
            """
        )
        cookie_text = st.text_area("BOSS Cookie JSON", height=180, placeholder='粘贴 Cookie-Editor 导出的 JSON 数组，例如 [{"name":"...","value":"...","domain":".zhipin.com"}]')
        if st.button("导入 BOSS Cookie 登录态"):
            if not cookie_text.strip():
                st.error("请先粘贴 BOSS Cookie JSON。")
            else:
                with st.spinner("正在导入 BOSS Cookie 登录态..."):
                    state_path = adapter.import_cookies_from_json(cookie_text)
                    is_logged_in = adapter.is_logged_in()
                st.session_state[login_status_key] = "已登录" if is_logged_in else "已保存，待验证"
                st.session_state[f"{adapter.platform_code}_login_state_updated_at"] = time.time()
                if is_logged_in:
                    st.success(f"BOSS Cookie 登录态已导入并验证成功：{state_path}")
                else:
                    st.warning(f"BOSS Cookie 已导入，但暂未通过沟通页验证：{state_path}")
                st.rerun()
        boss_c1, boss_c2 = st.columns(2)
        diagnose_seconds = boss_c1.number_input("BOSS诊断等待秒数", min_value=10, max_value=180, value=60, step=10)
        if boss_c1.button("BOSS 登录诊断"):
            with st.spinner("正在打开 BOSS 页面并记录导航状态，不会保存登录态..."):
                report = adapter.diagnose_login_navigation(wait_seconds=int(diagnose_seconds))
            st.session_state["boss_login_diagnostic_report"] = report
            st.info("诊断完成。")
        if boss_c2.button("清空 BOSS 诊断结果"):
            st.session_state.pop("boss_login_diagnostic_report", None)
            st.rerun()
        report = st.session_state.get("boss_login_diagnostic_report")
        if report:
            st.json(report)
