...
import recruitment_assistant.platforms.boss.adapter as boss_adapter_module
...
def get_platform_collect_meta(target_site: str | None) -> dict:
    target_site = target_site or "BOSS直聘"
    platform_meta = {
        "BOSS直聘": {
            "code": "boss",
            "name": "BOSS直聘",
            "task_prefix": "BOSS直聘采集",
            "target_url": "https://www.zhipin.com/web/chat/index",
            "module": boss_adapter_module,
            "class_name": "BossAdapter",
            "login_error_tokens": ["BOSS直聘自动登录未成功", "登录态不存在", "BOSS直聘页面异常", "当前仍停留在登录页面", "about:blank"],
        },
        "智联招聘": {
            "code": "zhilian",
            "name": "智联招聘",
    ...
    return platform_meta.get(target_site, platform_meta["BOSS直聘"])
...
def render_task_editor(task_config: dict, login_states: dict[str, bool], disabled: bool = False) -> dict:
    ...
    with task_banner:
        ...
        target_options = ["BOSS直聘", "智联招聘"]
        target_site = col1.selectbox(
            "目标网站",
            target_options,
            index=target_options.index(task_config.get("目标网站", "BOSS直聘")) if task_config.get("目标网站", "BOSS直聘") in target_options else 0,
            disabled=disabled,
        ...
        boss_state = "已保存" if login_states.get("BOSS直聘") else "未保存"
        ...
        col6.markdown(
            ...
            f'<div class="collect-info-value" style="font-size:12px;color:#6B7280;">BOSS直聘：{boss_state} ｜ 智联招聘：{zhilian_state}</div>'
...
def get_platform_login_states(account_name: str) -> dict[str, bool]:
    states = {}
    for target_site in ["BOSS直聘", "智联招聘"]:
        try:
...
def run_collect_task(task_config: dict, runtime: dict | None = None) -> None:
    ...
    try:
        ...
        if not has_login_state:
            ...
            if platform_code == "boss":
                raise RuntimeError("BOSS直聘登录态不存在：请先在系统设置中通过外部真实浏览器导入 BOSS Cookie 登录态。")
        ...
        def run_auto_collect_with_login_recovery() -> list[dict]:
            try:
                ...
                if platform_code == "boss":
                    raise RuntimeError("BOSS直聘登录态已失效：请先在系统设置中通过外部真实浏览器重新导入 BOSS Cookie 登录态。")
                adapter.login_manually(wait_seconds=900, keep_open=False)
                st.session_state[f"{platform_code}_login_status_{account_name}"] = "已登录"
...
current_target_site = pending_task.get("目标网站") or "BOSS直聘"
