...
from components.bridges import get_zhilian_bridge
...
st.set_page_config(page_title="智联采集", layout="wide", initial_sidebar_state="collapsed")
inject_vibe_style("智联招聘采集")
page_header(
    "智联招聘采集",
    "通过 Chrome 扩展在智联招聘页面内自动采集附件简历。",
    icon="icon/智联招聘.jpg",
)
...
def translate_zhilian_detail(value: str) -> str:
    if not value:
        return ""
...
def load_zhilian_history_rows() -> tuple[list[dict], int, int]:
    status_mapping = {
        "running": "运行中",
        "success": "成功",
        "cancelled": "已停止",
    ...
    with create_session() as session:
        ...
        task_rows = task_service.list_tasks(limit=50, platform_code="zhilian")
        success_task_count, success_resume_count = task_service.success_summary(platform_code="zhilian")
        rows = [
            {
                "批次ID": row.id,
...
@st.cache_data(ttl=30)
def load_zhilian_dedup_record_count() -> int:
    with create_session() as session:
        return BossCandidateRecordService(session).count_records("zhilian")


def load_zhilian_records_for_display() -> list[dict]:
    """读取 智联招聘 平台已归档候选人记录，按首次归档时间升序。"""
    try:
        with create_session() as session:
            return BossCandidateRecordService(session).list_records_for_display("zhilian")
    except Exception as exc:
        st.warning(f"已获取候选人列表读取失败：{exc}")
        return []


def render_zhilian_history_task_table() -> None:
    try:
        task_rows, success_task_count, success_resume_count = load_zhilian_history_rows()
    except Exception as exc:
...
def zhilian_task_elapsed_seconds(runtime_state: dict) -> float:
...
def build_log_summary(runtime_state: dict) -> str:
    ...
    elapsed_seconds = zhilian_task_elapsed_seconds(runtime_state)
    ...
    return (
        ...
        f"已扫描{format_zhilian_duration(elapsed_seconds)}，平均每人耗时{avg_seconds}秒"
    )
...
with st.container(border=True):
    ...
    page_text = "● 智联页面就绪" if page_ready else "○ 等待沟通页"
    ...
    status_cols[3].metric("当前网站入库记录数", current_dedup_total)
    status_cols[4].metric("最近事件", runtime.get("last_event_at") or "-")

    action_cols = st.columns([1.25, 1.25, 1.1, 5.4], gap="medium")
    action_cols[0].link_button("打开智联招聘页面", "https://rd5.zhaopin.com/")
    if action_cols[1].button("重新检测智联页面", disabled=not ws_connected):
        bridge.probe_page()
        st.rerun()
    if action_cols[2].button("重置日志信息", disabled=runtime.get("running", False)):
        bridge.reset_run()
        st.rerun()
    summary = st.session_state.get("zhilian_run_summary")
    if summary:
...
with st.container(border=True):
    ...
    if btn_cols[4].button("打开简历目录", use_container_width=True):
        ...
        save_dir = (settings.attachment_dir / "zhilian" / datetime.now().strftime("%Y%m%d")).resolve()
    ...
    with result_cols[0]:
        ...
        if logs:
            ...
            for entry in logs[-120:]:
                ...
                css_class = classify_zhilian_log(raw_msg, level)
                log_html += f'<div class="{css_class}">[{at}] {msg}</div>'
            if is_running:
                render_auto_scroll_html(f'<div class="boss-log-box">{log_html}</div>', "zhilian-log-bottom")
            else:
    ...
    with result_cols[1]:
        render_zhilian_history_task_table()

# --- Archived Candidates (full width, page bottom) ---
with st.container(border=True):
    try:
        current_dedup_total_for_summary = load_zhilian_dedup_record_count()
    except Exception:
    ...
    records = load_zhilian_records_for_display()
    if records:
        display_rows = [
        ...
        if del_cols[0].button(
            ...
            try:
                ...
                st.session_state["zhilian_record_delete_message"] = f"已删除 {deleted} 条记录"
            except Exception as exc:
                st.session_state["zhilian_record_delete_message"] = f"删除失败：{exc}"
            st.rerun()
        if del_cols[1].button(
            ...
            st.session_state["zhilian_confirm_delete_all"] = True
            st.rerun()
        msg = st.session_state.pop("zhilian_record_delete_message", None)
        if msg:
            del_cols[2].caption(msg)

        if st.session_state.get("zhilian_confirm_delete_all"):
            with st.container(border=True):
                st.warning("警告：这将删除智联招聘网站所有的已入库候选人记录！")
                confirm_cols = st.columns([1.4, 1.4, 5.2])
                if confirm_cols[0].button(
                    "确认全部删除",
                    type="primary",
                    use_container_width=True,
                    key="zhilian_confirm_delete_all_yes",
                ):
                    try:
                        with create_session() as session:
                            svc = BossCandidateRecordService(session)
                            deleted = svc.delete_all_by_platform("zhilian")
                        st.session_state["zhilian_record_delete_message"] = f"已全部删除 {deleted} 条记录"
                    except Exception as exc:
                        st.session_state["zhilian_record_delete_message"] = f"全部删除失败：{exc}"
                    st.session_state["zhilian_confirm_delete_all"] = False
                    st.rerun()
                if confirm_cols[1].button(
                    "取消",
                    use_container_width=True,
                    key="zhilian_confirm_delete_all_no",
                ):
                    st.session_state["zhilian_confirm_delete_all"] = False
                    st.rerun()
    else:
