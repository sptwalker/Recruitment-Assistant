...
def render_zhilian_history_task_table() -> None:
    ...
    st.markdown(
        '<div class="plain-section-title"><h3>智联招聘历史批次任务列表</h3><div class="collect-panel-stat">已成功执行{}次任务，共获取了{}份简历。</div></div>'.format(
            success_task_count,
...
with st.container(border=True):
    ...
    if action_cols[3].button("清除去重数据库", disabled=runtime.get("running", False)):
        try:
            deleted_count = bridge.clear_dedup_records()
            st.session_state["zhilian_dedup_clear_message"] = f"已清除 智联去重记录 {deleted_count} 条"
        except Exception as exc:
            st.session_state["zhilian_dedup_clear_message"] = f"清除 智联去重记录失败：{exc}"
        st.rerun()
    clear_message = st.session_state.get("zhilian_dedup_clear_message")
    if clear_message:
    ...
    with result_cols[1]:
        ...
        if candidates:
            ...
            for c in candidates[-30:]:
                ...
                detail = c.get("file", "") or c.get("reason_text", "") or translate_zhilian_detail(c.get("reason", ""))
                candidate_rows += (
                    "<tr>"
            ...
            if is_running:
                render_auto_scroll_html(table_html, "zhilian-candidate-bottom")
            else:
