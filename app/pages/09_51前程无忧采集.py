...
def render_boss_history_task_table() -> None:
    ...
    st.markdown(
        '<div class="plain-section-title"><h3>51前程无忧历史批次任务列表</h3><div class="collect-panel-stat">已成功执行{}次任务，共获取了{}份简历。</div></div>'.format(
            success_task_count,
...
with st.container(border=True):
    top_cols = st.columns([1.1, 1.1, 1.6, 4.2])
    ...
    top_cols[2].metric("当前去重记录数", current_dedup_total)
