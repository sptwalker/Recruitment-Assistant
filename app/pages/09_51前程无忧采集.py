...
from components.bridges import get_qiancheng_bridge
...
st.set_page_config(page_title="51前程无忧采集", layout="wide", initial_sidebar_state="collapsed")
inject_vibe_style("51前程无忧采集")
page_header(
    "51前程无忧采集",
...
def load_qiancheng_history_rows() -> tuple[list[dict], int, int]:
    status_mapping = {
        "running": "运行中",
        "success": "成功",
        "cancelled": "已停止",
    ...
    with create_session() as session:
        ...
        task_rows = task_service.list_tasks(limit=50, platform_code="qiancheng")
        success_task_count, success_resume_count = task_service.success_summary(platform_code="qiancheng")
        rows = [
            {
                "批次ID": row.id,
...
@st.cache_data(ttl=30)
def load_qiancheng_dedup_record_count() -> int:
    with create_session() as session:
        return BossCandidateRecordService(session).count_records("qiancheng")


def load_qiancheng_records_for_display() -> list[dict]:
    """读取 51前程无忧 平台已归档候选人记录，按首次归档时间升序。"""
    try:
        with create_session() as session:
            return BossCandidateRecordService(session).list_records_for_display("qiancheng")
...
def render_boss_history_task_table() -> None:
    try:
        task_rows, success_task_count, success_resume_count = load_qiancheng_history_rows()
    except Exception as exc:
...
def build_log_summary(runtime_state: dict) -> str:
    ...
    elapsed_seconds = qiancheng_task_elapsed_seconds(runtime_state)
...
def qiancheng_task_elapsed_seconds(runtime_state: dict) -> float:
...
with st.container(border=True):
    ...
    page_text = "● ehire页面就绪" if page_ready else "○ 等待沟通页"
    ...
    try:
        current_dedup_total = load_qiancheng_dedup_record_count()
    except Exception:
    ...
    action_cols = st.columns([1, 1, 1, 1, 4], gap="medium")
    action_cols[0].link_button("打开51Job网站", "https://ehire.51job.com/", use_container_width=True)
    if action_cols[1].button("重新检测51Job页面", disabled=not ws_connected, use_container_width=True):
        bridge.probe_page()
        st.rerun()
    if action_cols[2].button("重置日志信息", disabled=runtime.get("running", False), use_container_width=True):
        bridge.reset_run()
        st.rerun()
    if action_cols[3].button("清除去重数据库", disabled=runtime.get("running", False), use_container_width=True):
        try:
...
with st.container(border=True):
    ...
    if btn_cols[4].button("打开简历目录", use_container_width=True):
        ...
        boss_save_dir = (settings.attachment_dir / "51job" / datetime.now().strftime("%Y%m%d")).resolve()
...
with st.container(border=True):
    try:
        current_dedup_total_for_summary = load_qiancheng_dedup_record_count()
    except Exception:
    ...
    records = load_qiancheng_records_for_display()
    if records:
        display_rows = [
        ...
        if del_cols[0].button(
            ...
            try:
                ...
                st.session_state["qiancheng_record_delete_message"] = f"已删除 {deleted} 条记录"
            except Exception as exc:
                st.session_state["qiancheng_record_delete_message"] = f"删除失败：{exc}"
            st.rerun()
        if del_cols[1].button(
            ...
            st.session_state["qiancheng_confirm_delete_all"] = True
            st.rerun()
        msg = st.session_state.pop("qiancheng_record_delete_message", None)
        if msg:
            del_cols[2].caption(msg)

        if st.session_state.get("qiancheng_confirm_delete_all"):
            with st.container(border=True):
                st.warning("警告：这将删除51前程无忧网站所有的已入库候选人记录！")
                confirm_cols = st.columns([1.4, 1.4, 5.2])
                if confirm_cols[0].button(
                    "确认全部删除",
                    type="primary",
                    use_container_width=True,
                    key="qiancheng_confirm_delete_all_yes",
                ):
                    try:
                        with create_session() as session:
                            svc = BossCandidateRecordService(session)
                            deleted = svc.delete_all_by_platform("qiancheng")
                        st.session_state["qiancheng_record_delete_message"] = f"已全部删除 {deleted} 条记录"
                    except Exception as exc:
                        st.session_state["qiancheng_record_delete_message"] = f"全部删除失败：{exc}"
                    st.session_state["qiancheng_confirm_delete_all"] = False
                    st.rerun()
                if confirm_cols[1].button(
                    "取消",
                    use_container_width=True,
                    key="qiancheng_confirm_delete_all_no",
                ):
                    st.session_state["qiancheng_confirm_delete_all"] = False
                    st.rerun()
    else:
