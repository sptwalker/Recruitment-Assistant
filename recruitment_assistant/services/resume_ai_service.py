...
# ---- 失败降级通知槽（按会话隔离，供 Streamlit 页面 drain 后提示用户）----
# 用 ContextVar 而非模块级 list：多会话/多线程并发时各自独立，不串台。
from contextvars import ContextVar

_failover_notices_var: ContextVar["list[str] | None"] = ContextVar(
    "failover_notices", default=None
)


def _notices() -> list[str]:
    lst = _failover_notices_var.get()
    if lst is None:
        lst = []
        _failover_notices_var.set(lst)
    return lst


def push_failover_notice(failed_name: str, new_name: str, reason: str) -> None:
    _notices().append(
        f"⚠️ 接口「{failed_name}」调用失败（{reason}），已自动切换为「{new_name}」"
    )


def pop_failover_notices() -> list[str]:
    """取出并清空当前会话累积的降级通知。"""
    lst = _notices()
    out = list(lst)
    lst.clear()
...
def _failover_reason(exc: Exception | None) -> str:
    ...
    if isinstance(exc, APIStatusError):
        ...
        if code == 401:
            return "鉴权失败(401)"
        return f"服务端错误({code})"
    return type(exc).__name__ if exc else "未知错误"
...
class ResumeAIService:
    """AI 简历结构化解析 + 岗位匹配服务（支持多端点自动降级）。"""

    def __init__(self, api_key: str | None = None, base_url: str | None = None,
                 model: str | None = None, endpoints: list[dict] | None = None):
        """两种构造方式：
        - 单端点（向后兼容脚本/测试）：ResumeAIService(api_key, base_url, model)
        - 多端点降级链：ResumeAIService(endpoints=[{name,api_key,base_url,model}, ...])，
          按顺序为主接口 → 备用接口，主接口失败自动降级。
        """
        if endpoints:
            self._endpoints = [
                {
                    "name": ep.get("name", ep.get("base_url", "接口")),
                    "api_key": ep.get("api_key", ""),
                    "base_url": _normalize_base_url(ep.get("base_url", "")),
                    "model": ep.get("model", ""),
                }
                for ep in endpoints
            ]
    ...
    @classmethod
    def from_single(cls, api_key: str, base_url: str, model: str) -> "ResumeAIService":
    ...
    def _client_for(self, idx: int):
        if idx not in self._clients:
            ...
            self._clients[idx] = OpenAI(api_key=ep["api_key"], base_url=ep["base_url"])
        return self._clients[idx]

    @property
    def api_key(self) -> str:
        return self._endpoints[self._idx]["api_key"]

    @property
    def base_url(self) -> str:
        return self._endpoints[self._idx]["base_url"]

    @property
    def model(self) -> str:
        return self._endpoints[self._idx]["model"]

    @property
    def client(self):
        return self._client_for(self._idx)
    ...
    def _chat_completion(self, **kwargs):
        ...
        降级是「按调用」的：每次都从首选接口（index 0）开始，仅本次调用失败时才顺延到
        备用接口。不会永久改写实例状态——首选接口恢复后，下一次调用会再次优先它。
        （本服务实例被 @st.cache_resource 进程级共享，绝不能因一次瞬时抖动就永久切走。）
        """
        from openai import APIConnectionError, APIStatusError, APITimeoutError, RateLimitError

        TRANSIENT = (APITimeoutError, APIConnectionError, RateLimitError)
        PER_ENDPOINT_RETRIES = 2
        BACKOFF = [3, 6]
                ...
                try:
                    ...
                    if idx != 0:
                        ...
                        logger.warning("[AI降级] 「{}」→「{}」，原因：{}",
                                       prev["name"], ep["name"], _failover_reason(last_exc))
                        push_failover_notice(prev["name"], ep["name"], _failover_reason(last_exc))
                    return resp
                except TRANSIENT as exc:
                    last_exc = exc
                    if attempt < PER_ENDPOINT_RETRIES:
                        logger.warning("[AI调用] 端点「{}」第{}次重试 (等{}s): {} — {}",
                                       ep["name"], attempt + 1, BACKOFF[attempt],
                                       type(exc).__name__, exc)
                        _time.sleep(BACKOFF[attempt])
                        continue
                    break  # 本端点重试耗尽 → 降级下一端点
                except APIStatusError as exc:
                    last_exc = exc
                    code = getattr(exc, "status_code", 0)
                    # 兼容性兜底：部分 OpenAI 兼容端点（GLM/Claude 代理等）不支持
                    # response_format=json_object，会返回 400。此时去掉该参数在本端点原地重试，
                    # 靠 prompt 里的「严格只输出 JSON」约束即可。
                    if code == 400 and "response_format" in call_kwargs \
                            and "response_format" in str(exc).lower():
                        logger.warning("[AI调用] 端点「{}」不支持 response_format，去掉后原地重试", ep["name"])
                        call_kwargs.pop("response_format", None)
                        continue
                    if code >= 500 and attempt < PER_ENDPOINT_RETRIES:
                        _time.sleep(BACKOFF[attempt])
                        continue
                    if code >= 500 or code == 401:
                        break  # 5xx 耗尽 / 鉴权失败 → 降级下一端点
                    raise  # 其他 4xx（业务错误）直接抛出，降级无意义
        raise last_exc if last_exc else RuntimeError("无可用 AI 接口")

    def parse_resume_text(self, raw_text: str, source_name: str | None = None, retry: int = 1) -> CandidateCreate | None:
        """调 LLM 将纯文本简历结构化为 CandidateCreate。

        Args:
            raw_text: 简历原始文本
            source_name: 文件名
            retry: 失败重试次数，默认 1 次（端点级降级由 _chat_completion 兜底）
        """
