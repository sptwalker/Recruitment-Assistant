...
class ResumeArchiveService:
    ...
    def save_position_match(
        ...
        # core insert 绕过 before_flush 盖章：有租户上下文（走 API）时显式盖 tenant_id，
        # 否则匹配行 tenant_id 为空 → 被租户过滤（fail-closed）。无上下文（Streamlit/测试）→ None。
        from recruitment_assistant.storage import tenancy
        values["tenant_id"] = tenancy.current_tenant_id()
