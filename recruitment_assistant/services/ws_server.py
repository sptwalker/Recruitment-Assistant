...
class QianchengWSServer:
    """51 前程无忧（ehire）专用 WS 服务，监听 8766，与 BOSS 端 8765 完全隔离。

    结构与 BossWSServer 一致，单连接模式（新连接来时关旧连接）。
    """
    ...
    def start(self) -> None:
        ...
        for _ in range(50):
            ...
            deadline.wait(0.1)
        if self._startup_error:
            logger.error("Qiancheng WS 服务启动失败: {}", self._startup_error)
        elif self.is_listening:
            logger.info("Qiancheng WS 服务已启动: ws://{}:{}", self.host, self.port)
        else:
            self._startup_error = "启动超时：未能确认端口监听"
            logger.error("Qiancheng WS 服务启动失败: {}", self._startup_error)
    ...
    def stop(self) -> None:
        ...
        self._loop = None
        self.extension_ws = None
        logger.info("Qiancheng WS 服务已停止")

    def send_command(self, command: dict) -> None:
        if not self.is_extension_connected:
            logger.warning("Qiancheng 扩展未连接，无法发送指令: {}", command.get("type"))
            return
        if self._loop and self._command_queue:
            self._loop.call_soon_threadsafe(self._command_queue.put_nowait, command)
    ...
    def _run_loop(self) -> None:
        ...
        try:
            ...
            self._startup_error = str(exc)
            logger.exception("Qiancheng WS 服务线程异常")
        finally:
            self._server = None
    ...
    async def _handler(self, websocket) -> None:
        ...
        self.last_connected_at = datetime.now().isoformat(timespec="seconds")
        self.last_disconnect_reason = ""
        logger.info("Qiancheng 扩展已连接 #{}", connection_id)
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    logger.warning("Qiancheng 收到无效 JSON: {}", message[:100])
        except Exception as exc:
            disconnect_reason = str(exc)
            logger.info("Qiancheng 扩展连接断开 #{}: {}", connection_id, exc)
        finally:
            send_task.cancel()
            if self.extension_ws is websocket:
                self.extension_ws = None
                self.last_disconnected_at = datetime.now().isoformat(timespec="seconds")
                self.last_disconnect_reason = disconnect_reason
                logger.info("Qiancheng 扩展已断开 #{}", connection_id)
                if self.on_event:
                    self.on_event({
                        "type": "extension_disconnected",
                        "data": {
