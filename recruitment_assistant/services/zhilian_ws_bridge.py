...
class ZhilianWSBridge:
    ...
    def reset_run(self) -> None:
        ...
        log_dir = Path("logs") / "zhilian_extension" / now.strftime("%Y%m%d")
        ...
        self._log(
            ...
            "版本信息: "
            f"页面={APP_VERSION}；"
            f"智联招聘后端桥接={ZHILIAN_BRIDGE_VERSION}；"
            f"期望扩展={ZHILIAN_EXTENSION_EXPECTED_VERSION}；"
            f"期望内容脚本={ZHILIAN_CONTENT_SCRIPT_EXPECTED_VERSION}；"
            f"当前已连接扩展={self.runtime_state.get('extension_version') or '未连接'}",
        )
        self._write_event_log("module_versions", {
            "app_version": APP_VERSION,
            "zhilian_bridge_version": ZHILIAN_BRIDGE_VERSION,
            "expected_extension_version": ZHILIAN_EXTENSION_EXPECTED_VERSION,
            "expected_content_script_version": ZHILIAN_CONTENT_SCRIPT_EXPECTED_VERSION,
        ...
        if self.ws_server.is_extension_connected:
            ...
            self._log("info", "已请求扩展重新加载 智联招聘页面脚本")
            self.probe_page()

    def clear_dedup_records(self) -> int:
        with create_session() as session:
            deleted_count = BossCandidateRecordService(session).clear_records("zhilian")
        self._seen_candidate_records.clear()
        self.runtime_state["dedup_record_count"] = 0
        self._log("info", f"已清除 智联招聘 去重数据库: {deleted_count} 条")
        self._write_event_log("zhilian_dedup_records_cleared", {"deleted_count": deleted_count})
    ...
    def _create_crawl_task(self, config: dict) -> None:
        ...
        try:
            ...
            self._write_event_log("zhilian_crawl_task_created", {"task_id": task_id, "planned_count": planned_count})
    ...
    def _finish_crawl_task(self, status: str, error_message: str | None = None) -> None:
        ...
        try:
            ...
            self._write_event_log("zhilian_crawl_task_finished", {
                "task_id": task_id,
                "status": status,
                "success_count": success_count,
                "failed_count": failed_count,
        ...
        try:
            ...
            if run_started_at:
                try:
                    ...
                    run_started_ts = 0.0
            chrome_dir = Path.home() / "Downloads" / "智联招聘"
            settings = get_settings()
            today_archive = (settings.attachment_dir / "zhilian" / datetime.now().strftime("%Y%m%d")).resolve()
            ...
            if missed_files:
                self._log("warning", f"⚠ Chrome 下载目录有 {len(missed_files)} 个未归档的 智联招聘 PDF：")
    ...
    def probe_page(self) -> None:
        ...
        self.ws_server.send_command(command)
        self._write_event_log("command_sent", command)
        self._log("info", "已请求扩展重新检测 智联招聘页面")
    ...
    def _handle_event(self, event: dict) -> None:
        ...
        match event_type:
            ...
            case "zhilian_learning_required":
                self._log("highlight", "━━━ 智联招聘首次学习 ━━━")
                self._log("highlight", "本平台采集器尚未学习过 智联招聘 DOM。请在浏览器 智联招聘页面右上角的浮动 banner 中按提示完成以下 7 步：")
            ...
            case "zhilian_learning_step_failed":
                self._log("error", f"学习步骤失败：step={data.get('step', '?')}；原因={data.get('reason', '?')}")
            case "zhilian_learning_finished":
                if data.get("all_keys_captured"):
                    self._log("highlight", "🎉 7 步学习完成。请在 Chrome F12 → Application → Local Storage → rd5.zhaopin.com 复制 8 个 zhilian_* key 的 JSON 值发给开发者。")
                else:
                    self._log("warning", "学习结束但部分 key 未捕获，建议在 智联招聘页 F12 Console 手动删除 zhilian_* localStorage key 后重做。")
            ...
            case "content_script_inject_failed":
                self._log("error", f"注入 智联招聘页面脚本失败: {data.get('url', '')}")
            case "content_script_message_failed":
                self._log("error", f"智联招聘页面脚本通信失败: {data.get('error', '')} {data.get('url', '')}")
    ...
    def _build_boss_candidate_key(self, candidate_sig: str, candidate_info: dict[str, Any]) -> str:
        ...
        key = text_hash("|".join(["zhilian", "profile_name_age_education", name, age, education]))
        if key:
            return key
        return text_hash(f"zhilian|candidate_signature|{candidate_sig or ''}") or ""

    def _load_candidate_keys(self) -> set[str]:
        try:
            with create_session() as session:
                return BossCandidateRecordService(session).list_candidate_keys("zhilian")
        except Exception as exc:
    ...
    def _load_candidate_signatures(self) -> set[str]:
        try:
            with create_session() as session:
                signatures = BossCandidateRecordService(session).list_candidate_signatures("zhilian")
    ...
    def _save_resume(self, data: dict) -> None:
        ...
        if download_path:
            ...
            if source.exists():
                ...
                target_dir = project_root / "data" / "attachments" / "zhilian" / now.strftime("%Y%m%d")
